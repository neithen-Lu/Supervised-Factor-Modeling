...
N=40
for bic_c in np.arange(0.1,0.9,1e-2):
    print(bic_c)
    for r,T in itertools.product([4],[200,400,600,800,1000]):
        T0=9
        best_s=5
        best_combo = f'({r};{r};5)'
        df = pd.read_csv(f'../result/bic/season_ar_1212_N40_lr0.0005/csv/rank{r}T{T}T0{T0}.csv',header=0)
    ...
    #         best_s = 12; T0 = 67
        ...
        for col in df.columns:
            try:
                ...
                continue
            penalty = np.log(T-T0)*((int(r1)+int(r2)) * N + (np.log(T0)+int(r1)*int(r2))*int(s))/(T-T0)
            loss = df[col]
