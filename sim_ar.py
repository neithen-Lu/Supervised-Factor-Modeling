...
def main():
 ...
 # check and set other parameters based on args
    ...
    with open(f'{wkdir}/forecast_ar{args.ar_p}_p{args.dgp_p}_r1{r1}_r2{r2}_N{args.N}_T{args.T}.csv','a') as f:
        f.write(f'{args.seed},{args.n_rep},T{args.T}_N{args.N},{np.average(forecast_err[:,0])},{np.average(forecast_err[:,1])}\n')
    np.savetxt(f'{wkdir}/csv/forecast_ar{args.ar_p}_p{args.dgp_p}_r1{r1}_r2{r2}_N{args.N}_T{args.T}_seed{args.seed}.csv',forecast_err)
